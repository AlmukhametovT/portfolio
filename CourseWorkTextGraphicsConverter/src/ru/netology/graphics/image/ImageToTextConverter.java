...
public class ImageToTextConverter implements TextGraphicsConverter {
    ...
    public ImageToTextConverter() {
        ...
        this.maxHeight = 0;
        this.maxRatio = 0;
        this.schema = new ColorToCharConverter();
    }
    ...
    public String convert(String url) throws IOException, BadImageSizeException {
        ...
        // так можно получить картинку на ПК
//        BufferedImage img = ImageIO.read(new File(url));
        ...
        int newWidth = (maxWidth > 0) ? Math.min(width, maxWidth) : width;
        int newHeight = (maxHeight > 0) ? Math.min(height, maxHeight) : height;
        double widthRatio = width / newWidth;
        double heightRatio = height / newHeight;
        ...
        int crt[] = new int[3];
        char charForPixel[][] = new char[newHeight][newWidth];
        for (int h = 0; h < newHeight; h++) {
            for (int w = 0; w < newWidth; w++) {
                int color = bwRaster.getPixel(w, h, crt)[0];
                char c = schema.convert(color);
                charForPixel[h][w] = c; //запоминаем символ c, например, в двумерном массиве или как-то ещё на ваше усмотрение
            }
        }
        ...
        // от схемы.
        StringBuilder imageToText = new StringBuilder();
        for (int h = 0; h < newHeight; h++) {
            for (int w = 0; w < newWidth; w++) {
                imageToText.append(charForPixel[h][w]);
                imageToText.append(charForPixel[h][w]);
            }
            imageToText.append("\n");
        }
        String result = imageToText.toString();
